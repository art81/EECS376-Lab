...
#include "find_gearbox_bottom_top.cpp"
...
private:
    ...
    bool find_gearbox_bottom(float surface_height, geometry_msgs::PoseStamped &object_pose);
public:
...
void ObjectFinder::executeCB(const actionlib::SimpleActionServer<object_finder::objectFinderAction>::GoalConstPtr& goal) {
    ...
    if (!known_surface_ht) {
        ...
        table_ht = pclUtils_.find_table_height(MIN_X, MAX_X, MIN_Y, MAX_Y, TABLE_TOP_MIN, TABLE_TOP_MAX, 0.005);
